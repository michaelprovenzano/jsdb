class Table {
        ...
        for (let i = table.length - 1; i >= 0; i--) {
            ...
            if (query(thisRecord)) {
                ...
                keys.forEach(key => {
                    ...
                    this.table[thisRecord.id][key] = newRecord[key]
                });
                this.table[thisRecord.id].updated_at = new Date();
                records.push(this.table[thisRecord.id]);
            }
        }
    ...
    addRecord(record) {
        ...
        // Write record
        record.id = this.UUID();
        if (!this.indexed.id) this.indexed.id = {};
        this.indexed.id[record.id] = true;
...
class DB {
    ...
    load(data) {
        ...
        for (let key of sortedKeys) {
            ...
            let oldRecordIds = []
            records[key].forEach(r => oldRecordIds.push(r.id))
            ...
            let newRecordIds = []
            newRecords.forEach(r => newRecordIds.push(r.id));
